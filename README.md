...
Inside a project workspace, **Herdr Organizations: toggle project hierarchy sidebar** opens a right split at 30% by default. The recursive tree stays scoped to that project. Use Up/Down or `j`/`k` to move, Enter to focus or reopen a node, Space to collapse a coordinator, `s` to open settings, and `q` or Esc to close the split. The visible settings entry controls dock side, width, focus-on-open, auto-open, resolved nodes, status and role visibility, and strict toggle behavior. Settings live in `HERDR_PLUGIN_CONFIG_DIR/organization-sidebar.json`. The action does not write Herdr's global keybindings. The settings screen identifies the global `organizations` action and recommends `Ctrl+B`, then `Shift+H` for `organization-sidebar`.
